...
class UpdateController(QObject):
    ...
    def include_test_releases(self):
        value = self.settings.value("updates/include_test_releases", "-" in RELEASE_TAG)
        return value.casefold() not in {"0", "false", "no", "off"} if isinstance(value, str) else bool(value)

    def set_include_test_releases(self, enabled):
        self.settings.setValue("updates/include_test_releases", bool(enabled))
        self.schedule_automatic_check()

    def _check_identity(self):
        return f"{RELEASE_TAG}:{self.include_test_releases()}"
    ...
    def should_check_automatically(self):
        if not self.automatic_runtime_enabled():
            return False
        if self.settings.value("updates/last_check_identity", "") != self._check_identity():
            return True
    ...
    def check(self, *, silent=False):
        if self._reply is not None and self._reply.isRunning():
            # A user pressing Check during a startup check still gets its result.
            self._silent = self._silent and bool(silent)
            self._status("Checking GitHub releases…")
            return
        self._silent = bool(silent)
        self.available_release = None
        self._request_include_tests = self.include_test_releases()
        self._request_identity = self._check_identity()
        self._status("Checking GitHub releases…")
        self.busy_changed.emit(True)
        request = QNetworkRequest(QUrl(RELEASE_LIST_API_URL))
    ...
    def _handle_reply(self):
        ...
        if error != QNetworkReply.NetworkError.NoError:
            self._status("Could not reach GitHub. Check your connection and try again; analysis works offline.")
            if not self._silent:
                QMessageBox.information(
                    self.parent_window,
                    "Update check",
        ...
        self._retry_timer.stop()
        try:
            release = select_release(json.loads(data.decode("utf-8")),
                                     include_prereleases=self._request_include_tests)
        ...
        if release is None or not is_newer_release(release["tag"]):
            channel = "stable and test releases" if self._request_include_tests else "stable releases"
            self._status(f"No newer version in {channel}. Installed: {RELEASE_TAG}."
                         + (" Enable Include test releases to see preview builds." if not self._request_include_tests else ""))
            if not self._silent:
                QMessageBox.information(
                    self.parent_window,
                    "No update available",
...
class PrivacyPreferencesDialog(QDialog):
    ...
    def __init__(self, controller, parent=None):
        ...
        self.test_releases_check = QCheckBox("Include test releases (preview builds)")
        self.test_releases_check.setChecked(controller.include_test_releases())
        self.test_releases_check.toggled.connect(controller.set_include_test_releases)
        updates_layout.addWidget(self.test_releases_check)
        update_note = QLabel(
        ...
        self.update_status = QLabel(controller.status_text or f"Installed build: {RELEASE_TAG}")
    ...
    def _refresh_release(self, _message=""):
...
def show_about(parent=None):
    QMessageBox.about(
        parent,
        f"About {APP_NAME}",
        f"<b>{APP_NAME} Version {APP_VERSION}</b><br>Build: {RELEASE_TAG}<br><br>"
